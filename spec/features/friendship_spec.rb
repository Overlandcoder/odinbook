...
RSpec.describe 'Friendship', type: :feature do
  ...
  let!(:user3) { FactoryBot.create(:user, id: 3) }
  ...
  context 'when viewing own friend list' do
    before do
      visit user_friendships_path(user2.id)
    end

    it 'lists the correct user as a friend' do
      expect(page).to have_link "#{user1.username.capitalize}"
    end

    it 'does not show self as a friend' do
      expect(page).not_to have_link "#{user2.username.capitalize}"
    end
  end

  context 'when viewing friend list of friend' do
    before do
